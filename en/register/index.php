...
if (isset($_POST['btnSubmit'])) {
    ...
    // username not empty
    if (trim($username) === '') {
        $msgName = 'Please fill in a name';
        $allOk = false;
    }
    if (trim($email) === '') {
        $msgEmail = 'Please fill in your email';
        $allOk = false;
    }
    if (trim($password) === '') {
        $msgPassword = 'Please fill in your password';
        $allOk = false;
    }
    if (trim($passwordAgain) === '') {
        $msgPasswordAgain = 'Please fill in your password again';
        $allOk = false;
    }

    if (!(trim($password) === trim($passwordAgain)))
    {
        $msgPasswordAgain = 'The given passwords don"t match';
        $allOk = false;
    }
...
<head>
    ...
    <title>register | Multicultura</title>
    ...
    <div class="login">
        <h1>Register an account!</h1>
            <form action="<?php echo $_SERVER['PHP_SELF']; ?>" method="post">
                <div class="form-item">
                    <label for="username" id="username">Account name</label>
                ...
                <p>
                    <button type="submit" id="btnSubmit" name="btnSubmit" class="button btnlogin">Create account</button>
                    <a href="../login/" class="button btnregistreer">Do you already have an account</a>
                </p>
            </form>
        </div>
    </main>
